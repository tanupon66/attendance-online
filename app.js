...
async function seedAdmin() {
  ...
  try {
    ...
    if (!existing.empty) {
      throw new Error('มีแอดมินอยู่แล้ว ถ้าลืม PIN ให้แก้ใน Firebase Console');
    }
...
async function refreshMyStatus() {
  ...
  $('todayStatus').textContent = !last
    ? 'วันนี้ยังไม่ได้ลงเวลา'
    : `ล่าสุด: ${last.type === 'IN' ? 'เข้างาน' : 'ออกงาน'} เวลา ${
        last.createdAt?.toDate ? fmtDateTime(last.createdAt.toDate()) : '-'
      }`;
}
...
async function loadMyHistory() {
  ...
  $('myHistory').innerHTML =
    snap.docs.map(d => renderAttendanceItem({ id: d.id, ...d.data() }, false)).join('') ||
...
async function autoClock() {
  ...
  const last = rows.at(-1);
  await clock(!last || last.type === 'OUT' ? 'IN' : 'OUT');
}
...
async function loadTodayAdmin() {
  ...
  $('todayList').innerHTML =
    rows.map(r => renderAttendanceItem(r, true)).join('') ||
    '<p class="muted">วันนี้ยังไม่มีข้อมูล</p>';
}
...
function renderAttendanceItem(r, admin) {
  ...
  const geo =
    r.inGeofence === null || r.inGeofence === undefined
      ? ''
      : `<span class="badge ${r.inGeofence ? 'good' : 'bad'}">${
          r.inGeofence ? 'ในพื้นที่' : 'นอกพื้นที่'
        } ${r.distanceMeters ? Math.round(r.distanceMeters) + 'm' : ''}</span>`;

  const img = r.photoURL
    ? `<img src="${r.photoURL}" class="thumb" loading="lazy" />`
    : '';
  ...
  return `
    <div class="item">
      <div class="item-head">
        <div>
          ${img}
          <b>${safeText(r.fullName || r.employeeCode)}</b><br>
          <span class="muted">
            ${safeText(r.employeeCode)} • ${r.type === 'IN' ? 'เข้างาน' : 'ออกงาน'} • ${dt}
          </span><br>
          ${geo}
          <span class="badge">${safeText(r.source || 'EMPLOYEE')}</span>
        </div>
      </div>
      <div class="row-actions">
        ${r.mapUrl ? `<a href="${r.mapUrl}" target="_blank"><button class="ghost">เปิดแผนที่</button></a>` : ''}
        ${admin ? `<button class="ghost" onclick="copyText('${r.id}')">คัดลอก ID</button>` : ''}
      </div>
    </div>
  `;
}
  ...
  $('employeeList').innerHTML =
    rows.map(e => `
      <div class="item">
        <div class="item-head">
          <div>
            <b>${safeText(e.employeeCode)} - ${safeText(e.fullName)}</b><br>
            <span class="muted">${safeText(e.department)} • ${safeText(e.position)} • ${e.role}</span><br>
            <span class="badge ${e.active ? 'good' : 'bad'}">${e.active ? 'ใช้งาน' : 'ปิด'}</span>
          </div>
        </div>
        <div class="row-actions">
          <button onclick="editEmployee('${e.id}')" class="warning">แก้ไข</button>
          <button onclick="toggleEmployee('${e.id}', ${e.active ? 'false' : 'true'})" class="ghost">
            ${e.active ? 'ปิดใช้งาน' : 'เปิดใช้งาน'}
          </button>
        </div>
      </div>
    `).join('') ||
    '<p class="muted">ยังไม่มีพนักงาน</p>';
}
...
window.editEmployee = async (id) => {
  ...
  $('empStdStart').value = e.standardStart || '08:00';
  $('empStdEnd').value = e.standardEnd || '17:00';
  $('empActive').checked = !!e.active;
  $('empPin').value = '';
...
window.toggleEmployee = async (id, active) => {
  ...
  await logAudit('TOGGLE_EMPLOYEE', { id, active });
  loadEmployees();
};
...
async function saveEmployee() {
  ...
  const data = {
    ...
    hourlyRate: Number($('empHourly').value || 0),
    dailyRate: Number($('empDaily').value || 0),
    otMultiplier: Number($('empOt').value || 1.5),
    standardStart: $('empStdStart').value || '08:00',
    standardEnd: $('empStdEnd').value || '17:00',
    updatedAt: firebase.firestore.FieldValue.serverTimestamp()
  };
  ...
  await logAudit(id ? 'UPDATE_EMPLOYEE' : 'ADD_EMPLOYEE', { code });
  clearEmployeeForm();
  loadEmployees();
...
async function loadAttendance() {
  ...
  lastAttendanceRows = snap.docs
    .map(d => ({ id: d.id, ...d.data() }))
    .sort((a, b) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0));

  $('attendanceList').innerHTML =
    lastAttendanceRows.map(r => renderAttendanceItem(r, true)).join('') ||
    '<p class="muted">ไม่พบข้อมูล</p>';
}

async function saveCorrection() {
  const code = $('corrEmpCode').value.trim();
  const reason = $('corrReason').value.trim();

  if (!code || !reason) return toast('กรอกรหัสพนักงานและเหตุผล');
  ...
  if (empSnap.empty) return toast('ไม่พบพนักงาน');

  const doc = empSnap.docs[0];
  const e = { id: doc.id, ...doc.data() };
  const overrideTime = `${$('corrDate').value} ${$('corrTime').value}:00`;
  ...
  await db.collection('attendance').add({
    ...
    dateKey: $('corrDate').value,
    overrideTime,
    reason,
    createdAt: firebase.firestore.FieldValue.serverTimestamp(),
    correctedBy: currentEmployee.employeeCode
  });

  await logAudit('CORRECT_ATTENDANCE', { employeeCode: code, overrideTime, reason });
...
async function loadHolidays() {
  ...
  $('holidayList').innerHTML =
    snap.docs.map(d => {
      ...
      return `
        <div class="item">
          <b>${safeText(h.dateKey)}</b> ${safeText(h.description || '')}
          <div class="row-actions">
            <button class="danger" onclick="deleteHoliday('${h.id}')">ลบ</button>
          </div>
        </div>
      `;
    }).join('') ||
    '<p class="muted">ยังไม่มีวันหยุด</p>';
}

async function addHoliday() {
  if (!$('holidayDate').value) return toast('เลือกวันที่');

  await db.collection('holidays').add({
    dateKey: $('holidayDate').value,
    description: $('holidayDesc').value.trim(),
    isPaid: true,
    createdAt: firebase.firestore.FieldValue.serverTimestamp()
  });

  await logAudit('ADD_HOLIDAY', { date: $('holidayDate').value });
  $('holidayDesc').value = '';
  loadHolidays();
}
...
window.deleteHoliday = async (id) => {
  ...
  await logAudit('DELETE_HOLIDAY', { id });
  loadHolidays();
};
...
async function saveSettings() {
  ...
  await logAudit('UPDATE_SETTINGS', companySettings);
  await loadSettings();
...
async function useCurrentLocation() {
  ...
  $('setLat').value = p.lat.toFixed(6);
  $('setLng').value = p.lng.toFixed(6);
}
...
async function runPayroll() {
  ...
  lastPayrollRows = employees
    .map(e => calcPayroll(e, records.filter(r => r.employeeId === e.id)))
    .filter(r => r.totalDays > 0);

  $('payrollList').innerHTML =
    lastPayrollRows.map(r => `
      <div class="item">
        <b>${safeText(r.employeeCode)} ${safeText(r.fullName)}</b><br>
        <span class="muted">
          วันทำงาน ${r.totalDays} • ชม.ปกติ ${r.regularHours.toFixed(2)}
          • OT ${r.otHours.toFixed(2)} • สาย ${r.lateMinutes} นาที
        </span><br>
        <b>สุทธิ ${r.netPay.toFixed(2)} บาท</b>
      </div>
    `).join('') ||
    '<p class="muted">ไม่พบข้อมูลเงินเดือน</p>';
}

function recTime(r) {
  if (r.overrideTime) return new Date(r.overrideTime.replace(' ', 'T'));
...
function calcPayroll(e, rows) {
  ...
  Object.entries(byDate).forEach(([date, list]) => {
    ...
    const ins = list.filter(r => r.type === 'IN').map(recTime).filter(Boolean);
    const outs = list.filter(r => r.type === 'OUT').map(recTime).filter(Boolean);
    ...
    otHours += Math.max(0, hrs - 8);

    if (e.standardStart) {
      const std = new Date(`${date}T${e.standardStart}:00`);
      if (start > std) lateMinutes += Math.round((start - std) / 60000);
    }
  });

  const hourly = Number(e.hourlyRate || 0) || (Number(e.dailyRate || 0) / 8);
  const otMult = Number(e.otMultiplier || 1.5);

  const regularPay = regularHours * hourly;
  const otPay = otHours * hourly * otMult;
  const lateDeduction = lateMinutes * (hourly / 60);
...
async function loadAudit() {
  ...
  $('auditList').innerHTML =
    snap.docs.map(d => {
      const l = d.data();
      const dt = l.createdAt?.toDate ? fmtDateTime(l.createdAt.toDate()) : '-';

      return `
        <div class="item">
          <b>${safeText(l.action)}</b><br>
          <span class="muted">${dt} • ${safeText(l.actorCode)} ${safeText(l.actorName)}</span><br>
          <code>${safeText(JSON.stringify(l.details || {}))}</code>
        </div>
      `;
    }).join('') ||
    '<p class="muted">ยังไม่มี log</p>';
}

function exportCsv(filename, rows) {
  if (!rows.length) return toast('ไม่มีข้อมูลให้ export');
...
function exportAttendance() {
  const rows = lastAttendanceRows.map(r => ({
    ...
    type: r.type,
    dateKey: r.dateKey,
    time: r.createdAt?.toDate ? fmtDateTime(r.createdAt.toDate()) : r.overrideTime,
    lat: r.latitude,
    lng: r.longitude,
  ...
  exportCsv(`attendance-${todayKey()}.csv`, rows);
}

function exportPayroll() {
  exportCsv(`payroll-${todayKey()}.csv`, lastPayrollRows);
}

function bind() {
  $('loginBtn').onclick = login;
  $('seedAdminBtn').onclick = seedAdmin;
  $('logoutBtn1').onclick = logout;
  $('logoutBtn2').onclick = logout;

  $('startCameraBtn').onclick = startCamera;
  $('captureBtn').onclick = () => captureSelfie().catch(e => toast(e.message, 5000));
  $('clockInBtn').onclick = () => clock('IN');
  $('clockOutBtn').onclick = () => clock('OUT');
  $('autoClockBtn').onclick = autoClock;

  document.querySelectorAll('.tabs button').forEach(b => {
    b.onclick = () => switchTab(b.dataset.tab);
  });

  $('refreshTodayBtn').onclick = loadTodayAdmin;
  $('saveEmployeeBtn').onclick = saveEmployee;
  $('clearEmployeeBtn').onclick = clearEmployeeForm;
  $('loadAttendanceBtn').onclick = loadAttendance;
  $('saveCorrectionBtn').onclick = saveCorrection;
  $('exportAttendanceBtn').onclick = exportAttendance;
  $('runPayrollBtn').onclick = runPayroll;
  $('exportPayrollBtn').onclick = exportPayroll;
  $('addHolidayBtn').onclick = addHoliday;
  $('useCurrentLocationBtn').onclick = () => useCurrentLocation().catch(e => toast(e.message, 5000));
