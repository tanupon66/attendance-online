...
export function shell({ employee, title, subtitle, active="dashboard", body="" }){
  ...
  const adminItems = [
    ["dashboard", t("dashboard"), "🏠"], ["employees", t("employees"), "👥"], ["attendance", t("attendance"), "🕒"], ["attendanceTools", "เครื่องมือเวลา", "🧰"], ["geofenceSettings", "ตำแหน่งบริษัท", "📍"],
    ["summary", t("summary"), "📊"], ["leave", t("leave"), "🌴"], ["calendar", t("calendar"), "📅"],
    ["payroll", t("payroll"), "💰"], ["notifications", t("notifications"), "🔔"], ["profile", t("profile"), "👤"]
  ];
  const userItems = [
    ["dashboard", t("dashboard"), "🏠"], ["clock", t("attendance"), "📷"], ["ot", "ขอ OT", "⏱️"], ["summary", t("summary"), "📊"],
    ["leave", t("leave"), "🌴"], ["calendar", t("calendar"), "📅"], ["payroll", t("payroll"), "💰"],
  ...
  return `
    <main class="app-shell">
      ...
      <section class="main-area">
        <header class="topbar">
          <div><p class="eyebrow">${safeText(nowText())}</p><h1>${safeText(title)}</h1><p class="muted">${safeText(subtitle || "")}</p></div>
          <div class="top-actions">${langSelector()}<button id="notificationBtn" class="icon-btn" title="${t("notifications")}">🔔</button><span class="pill">${safeText(employee.employeeCode || "-")}</span><button id="logoutBtn" class="danger small-btn">${t("logout")}</button></div>
        </header>
        <div class="mobile-nav">${items.map(([key,label,icon])=>`<button class="nav-btn ${active===key?"active":""}" data-route="${key}"><span>${icon}</span><small>${label}</small></button>`).join("")}</div>
